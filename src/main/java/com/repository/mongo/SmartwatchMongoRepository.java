...
@Singleton
public class SmartwatchMongoRepository implements CrudMongoRepository<Smartwatch> {
    ...
    private final MongoCollection<Document> documents = mongodb.getCollection("Smartwatch");
    ...
    @Override
    public Smartwatch getById(String id) {
        Document document = documents.find(eq("id", id)).first();
        return gson.fromJson(document.toJson(), Smartwatch.class);
    }
}

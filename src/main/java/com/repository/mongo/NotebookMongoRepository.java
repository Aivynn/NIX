...
@Singleton
public class NotebookMongoRepository implements CrudMongoRepository<Notebook> {
    ...
    private final MongoCollection<Document> documents = mongodb.getCollection("Notebook");

    @Override
    public void save(Notebook product) {
    ...
    public Notebook getById(String id) {
        Document document = documents.find(eq("id", id)).first();
        return gson.fromJson(document.toJson(), Notebook.class);
    }
}
